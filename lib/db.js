...
export function renderHtml(html, settings = {}, { globalHeaderHtml = null, seo = null } = {}) {
  ...
  // Marca o <body> com a slug da página (ex.: ce-slug-pisos) para permitir
  // regras CSS escopadas por página sem tocar no HTML salvo no banco.
  if (seo && seo.path) {
    const slugPart = String(seo.path).replace(/^\/+|\/+$/g, "").split("/")[0] || "home";
    const slugCls = `ce-slug-${slugPart.toLowerCase().replace(/[^a-z0-9]+/g, "-")}`;
    if (!new RegExp(`<body\\b[^>]*${slugCls}`, "i").test(rendered)) {
      if (/<body\b[^>]*\bclass="/i.test(rendered)) rendered = rendered.replace(/<body\b([^>]*)\bclass="/i, `<body$1class="${slugCls} `);
      else rendered = rendered.replace(/<body\b/i, `<body class="${slugCls}"`);
    }
  }
  ...
  if (/fab-grid/.test(rendered)) {
    tags.push(`<script id="casa-estampa-fab-equal">${buildFabEqualScript()}</script>`);
    tags.push(`<script id="casa-estampa-fab-redesign">${buildFabRedesignScript()}</script>`);
  }
...
function sanitizeLegacyCarouselScripts(html) {
  ...
  // ── FAQ legado duplicado: o script embutido "ce-faq-js" (salvo no HTML de
  // cortinas e outras páginas) registra seu próprio toggle nos mesmos títulos
  // que o script global casa-estampa-faq-toggle — o clique alternava 2x e a
  // resposta nunca abria. O global cobre .elementor-toggle sozinho: remover.
  sanitized = sanitized.replace(
    /<script\b[^>]*\bid=["']ce-faq-js["'][^>]*>[\s\S]*?<\/script\s*>/gi,
    ""
  );
  // ── Capa errada de vídeo: remove poster apontando p/ imagem WhatsApp antiga
  // (ex.: corporativo mostrava foto de cadeiras no vídeo do Fairmont) — sem
  // poster o player exibe o 1º frame do próprio vídeo; garante preload mínimo.
  sanitized = sanitized.replace(
    /\s+poster=(["'])[^"']*WhatsApp-Image-2026-07-20-at-21\.18\.28\.jpeg\1/gi,
    ""
  );
  sanitized = sanitized.replace(
    /<video\b(?![^>]*\bpreload=)/gi,
...
export function buildGlobalCss(settings) {
  ...
  // ── Heroes: só FOTO + BOTÕES — todos os textos do hero ficam ocultos porque
  // já estão escritos nas capas novas (lote capas 15/09). Vale p/ celular, PC e
  // preview do editor (mesmo CSS global). Botões e logo nunca são tocados.
  rules.push(".hero .hero-title,.hero .hero-subtitle,.hero .hero-sub,.hero .hero-desc,.hero .hero-quote,.hero .hero-label,.hero .hero-bg-word,.hero .hero-title-line2{display:none!important;}");
  // NOTA 15/09-4: NÃO voltar a esconder widgets do hero via seletor global
  // `:first-of-type` — ele apagou textos do site inteiro (papéis/cortinas/
  // persianas). Heroes Elementor serão tratados com classe marcada no banco.
  // Textos do hero Elementor marcados no banco (classe ce-hero-hide): ficam
  // ocultos porque o texto já está na capa. Só FOTO + BOTÕES (15/09-4).
  rules.push(".ce-hero-hide{display:none!important;}");
  // ── Heroes: botões mais para baixo (lote capas 15/09) — respiro acima do
  // bloco de botões em todos os heroes (custom .hero-actions/.hero-ctas e
  // Elementor .hero-btns do primeiro top-section). Só o bloco desce; o segundo
  // botão mantém o gap original (margem no container, não nos botões).
  rules.push(".hero .hero-actions,.hero .hero-ctas{margin-top:min(30vh,280px)!important;}");
  rules.push(".elementor > section.elementor-top-section:first-of-type .hero-btns{margin-top:min(30vh,280px)!important;}");
  // Papéis: botões ainda mais para baixo p/ não cobrir o texto da capa
  // ("Suporte do início ao fim"). Só papéis — as outras abas estão ok.
  rules.push("body.ce-slug-papeis-de-parede .elementor .elementor-top-section:first-of-type .hero-btns{margin-top:min(48vh,410px)!important;}");
  ...
  // ── Motorizadas no mobile: a camada de vídeo de fundo vira fluxo normal (nada de absolute/inset:0 com overflow cortando) — vídeo inteiro, sem corte nem achatamento
  rules.push("@media(max-width:768px){.mot-video-ph [data-ce-bg-video],.mot-video [data-ce-bg-video]{position:relative!important;inset:auto!important;top:auto!important;right:auto!important;bottom:auto!important;left:auto!important;height:auto!important;overflow:visible!important;} .mot-video-ph [data-ce-bg-video] video,.mot-video [data-ce-bg-video] video{width:100%!important;height:auto!important;max-height:none!important;object-fit:contain!important;display:block!important;}}");
  ...
  // ── FAQ: um único "+" por pergunta — esconde TODOS os símbolos embutidos
  // (spans closed/opened + .ce-faq-icon + <i>/<svg> do Elementor, ex. cortinas e
  // persianas), pois o "+" já é desenhado via ::after no ícone; sem isso o clique
  // mostra 2 símbolos juntos (Images 2 e 4 da cliente). Seletor com `body` para
  // vencer o CSS embutido da página (.ce-exact-page .faq-list ...) e sem :has()
  // (Safari antigo do iPhone descarta a regra inteira quando usa :has).
  rules.push("body .elementor-toggle .elementor-toggle-icon > *,body .elementor-toggle .elementor-toggle-icon *{display:none!important;}");
  rules.push("body .elementor-toggle .elementor-toggle-icon .ce-faq-icon,body .elementor-toggle .elementor-toggle-icon-closed,body .elementor-toggle .elementor-toggle-icon-opened{display:none!important;}");
  rules.push("body .faq-list .elementor-toggle .elementor-toggle-icon{width:24px!important;height:24px!important;min-width:24px!important;flex-basis:24px!important;}");
  // ── Papéis "Veja como fica na prática" (insp-track): sem laterais escuras e
  // fotos maiores — 1 foto por vez no celular (sem slide vizinho escuro ao lado)
  // e nunca escurecer/apagar slides; legenda mantém só o véu inferior.
  // (As laterais escuras vêm do WRAP .insp-carousel-wrap::before/::after, não do
  // slide — por isso a regra antiga no slide nunca resolveu. Image 1 cliente.)
  rules.push(".insp-carousel-wrap::before,.insp-carousel-wrap::after{display:none!important;content:none!important;background:none!important;box-shadow:none!important;width:0!important;}");
  rules.push(".insp-track .insp-slide{filter:none!important;opacity:1!important;-webkit-mask-image:none!important;mask-image:none!important;}");
  rules.push(".insp-track .insp-slide::before,.insp-track .insp-slide::after{background:none!important;box-shadow:none!important;}");
  rules.push("@media(max-width:768px){.insp-track .insp-slide{flex:0 0 100%!important;width:100%!important;max-width:100%!important;min-width:0!important;scroll-snap-align:center!important;min-height:430px!important;}}");
  rules.push("@media(min-width:769px){.insp-track .insp-slide{width:220px!important;height:300px!important;min-height:300px!important;}.insp-track .insp-slide.wide{width:340px!important;}}");
  // ── Corporativo "Como trabalhamos" + Papéis/Persianas "Como funciona": no
  // celular empilha verticalmente (nada correndo pro lado, nada espremido).
  // Substitui o 5-col espremido (letras sobrepostas) e o scroll horizontal.
  rules.push("@media(max-width:768px){body .proc-steps{display:grid!important;grid-template-columns:1fr!important;gap:20px!important;overflow:visible!important;padding-bottom:0!important;}body .proc-steps::before{display:none!important;}body .proc-steps>*,body .proc-steps .proc-step{grid-column:1/-1!important;min-width:0!important;width:100%!important;max-width:100%!important;flex:none!important;padding:0!important;text-align:left!important;scroll-snap-align:none!important;}body .proc-steps .elementor-section,body .proc-steps .elementor-container{width:100%!important;max-width:100%!important;}body .proc-steps .elementor-column{min-width:0!important;}body .processo .steps{display:flex!important;flex-direction:column!important;flex-wrap:nowrap!important;gap:16px!important;overflow:visible!important;padding-bottom:0!important;}body .processo .steps::before{display:none!important;}body .processo .step{flex:none!important;width:100%!important;max-width:100%!important;min-width:0!important;scroll-snap-align:none!important;}}");
  // ── Corporativo logos: maiores + placeholder claro (Pio XI) com chip escuro
  rules.push("body .caso-logo img,body .caso-logo-img img{height:64px!important;max-height:64px!important;max-width:210px!important;}");
  rules.push("@media(max-width:768px){body .caso-logo{padding:16px 22px!important;}body .caso-logo img,body .caso-logo-img img{height:60px!important;max-height:60px!important;max-width:200px!important;}}");
  rules.push("img[src*=\"pio-xi-placeholder\"]{background:#241d12!important;border-radius:6px!important;padding:10px 14px!important;}");
  // ── Pisos "Como trabalhamos": títulos dos passos centralizados (pedido cliente)
  rules.push("body.ce-slug-pisos .processo .proc-title,body.ce-slug-pisos .processo .step-nome{text-align:center!important;}");
  ...
  // Escopado SÓ p/ papéis (textos curtos); nas demais páginas os passos empilham (lote 15/09-2).
  rules.push("@media(max-width:768px){body.ce-slug-papeis-de-parede .proc-steps{grid-template-columns:repeat(5,minmax(0,1fr))!important;gap:2px!important;}.proc-steps::before{display:block!important;top:15px!important;left:22px!important;right:22px!important;}.proc-step{padding:0 3px!important;}.proc-num{width:30px!important;height:30px!important;min-width:30px!important;min-height:30px!important;margin:0 auto 12px!important;font-size:12px!important;}.proc-title{font-size:11px!important;line-height:1.25!important;margin-bottom:6px!important;}.proc-desc{font-size:9.5px!important;line-height:1.5!important;}}");
  // ── Padrão 1-5 do papel de parede (base desktop, global): persianas/pisos usam as mesmas classes
  // — valores iguais aos da página de papéis, com fallback caso a página não defina as variáveis.
  rules.push(".proc-steps{display:grid!important;grid-template-columns:repeat(5,1fr)!important;gap:2px!important;margin-top:28px!important;position:relative!important;}.proc-steps::before{content:''!important;position:absolute!important;top:20px!important;left:40px!important;right:40px!important;height:1px!important;background:linear-gradient(to right,#c8960c,#c2a57a,#c8960c)!important;background:linear-gradient(to right,var(--mustard,#c8960c),var(--fendi,#c2a57a),var(--mustard,#c8960c))!important;opacity:.25!important;}.proc-step{text-align:center!important;padding:0 16px!important;}.proc-num{width:40px!important;height:40px!important;background:#c8960c!important;background:var(--mustard,#c8960c)!important;display:flex!important;align-items:center!important;justify-content:center!important;font-size:14px!important;font-weight:600!important;color:#fff!important;margin:0 auto 20px!important;position:relative!important;z-index:1!important;}.proc-title{font-family:'Playfair Display',serif!important;font-size:15px!important;font-weight:700!important;color:#f5eed8!important;color:var(--text-light,#f5eed8)!important;margin-bottom:8px!important;line-height:1.3!important;}.proc-desc{font-size:13px!important;color:#a89370!important;color:var(--text-muted,#a89370)!important;font-weight:300!important;line-height:1.6!important;margin-top:6px!important;}");
  // ── Corporativo/Pisos/Persianas: "Como trabalhamos / Seu projeto passo a passo"
  // EMPILHADO no mobile (1 por linha, legível) — pedido cliente lote 15/09-2
  // ("correndo pro lado" e "letras juntas" quando ficava lado a lado/scroll).
  rules.push("@media(max-width:768px){body .processo{padding:44px 20px!important;}body .processo .steps{display:flex!important;flex-direction:column!important;flex-wrap:nowrap!important;gap:30px!important;margin-top:32px!important;overflow:visible!important;padding-bottom:0!important;}body .processo .steps::before{display:none!important;}body .processo .step{flex:none!important;width:100%!important;max-width:100%!important;min-width:0!important;padding:0!important;text-align:center!important;background:transparent!important;border-top:none!important;scroll-snap-align:none!important;}body .processo .step-dot{margin:0 auto 14px!important;}body .processo .step-nome{font-size:17px!important;text-align:center!important;}body .processo .step-desc{font-size:14px!important;line-height:1.65!important;text-align:center!important;}}");
  // ── Persianas/Pisos/Corporativo "Como funciona" (.proc-steps): empilhado no
  // mobile, 1 passo por linha com número em cima (lote 15/09-2). Papéis mantém
  // os 5 lado a lado (regra escopada acima, pedido anterior da cliente).
  rules.push("@media(max-width:768px){body .proc-steps{display:grid!important;grid-template-columns:1fr!important;gap:30px!important;overflow:visible!important;padding-bottom:0!important;}body .proc-steps::before{display:none!important;}body .proc-steps .proc-step{flex:none!important;width:100%!important;max-width:100%!important;min-width:0!important;padding:0 16px!important;text-align:center!important;scroll-snap-align:none!important;}body .proc-steps .proc-num{margin:0 auto 14px!important;}body .proc-steps .proc-title{font-size:17px!important;line-height:1.3!important;}body .proc-steps .proc-desc{font-size:14px!important;line-height:1.65!important;}}");
  // ── Lote cliente 09/09: vídeos persianas enquadro + capa corporativo + nº4 + home carrosséis + logos + bordas + sublinhados
  // A) Persianas/cortinas motorizadas: nunca cortar (contain, altura auto) — mobile, desktop e editor usam o mesmo CSS
  rules.push(".mot-videos{overflow:visible!important;max-width:100%!important;box-sizing:border-box!important;}");
  rules.push(".mot-videos .elementor-widget-wrap{box-sizing:border-box!important;max-width:100%!important;}");
  rules.push(".mot-video,.mot-video-ph,[data-video-slot]{width:100%!important;max-width:100%!important;height:auto!important;min-height:0!important;aspect-ratio:auto!important;overflow:hidden!important;background:#000!important;box-sizing:border-box!important;}");
  rules.push(".mot-video video,.mot-video-ph video,[data-video-slot] video{width:100%!important;height:auto!important;aspect-ratio:auto!important;object-fit:contain!important;max-height:none!important;display:block!important;background:#000!important;}");
  rules.push("@media(max-width:768px){.mot-video video,.mot-video-ph video,[data-video-slot] video{max-height:82vh!important;}}");
  // A2) Persianas/cortinas: o wrapper de vídeo de fundo vira fluxo normal em
  // TODAS as larguras (não só mobile) e sem :has() — vídeo vertical (WhatsApp)
  // aparece INTEIRO, sem corte, no celular, computador e editor (Image 3).
  // Vence os inlines salvos no banco (aspect-ratio:16/9 na coluna + absolute
  // inset:0 no wrapper), que cortavam o vídeo numa caixa 16/9.
  rules.push("body .mot-video-ph,body .mot-video{height:auto!important;min-height:0!important;aspect-ratio:auto!important;}");
  rules.push("body .mot-video-ph [data-ce-bg-video],body .mot-video [data-ce-bg-video]{position:relative!important;inset:auto!important;top:auto!important;right:auto!important;bottom:auto!important;left:auto!important;width:100%!important;height:auto!important;min-height:0!important;aspect-ratio:auto!important;overflow:visible!important;pointer-events:auto!important;}");
  rules.push("body .mot-video-ph [data-ce-bg-video] video,body .mot-video [data-ce-bg-video] video{width:100%!important;height:auto!important;min-height:0!important;aspect-ratio:auto!important;object-fit:contain!important;max-height:78vh!important;display:block!important;}");
  rules.push("@media(max-width:768px){body .mot-video-ph [data-ce-bg-video] video,body .mot-video [data-ce-bg-video] video{max-height:82vh!important;}}");
  // A3) Slots de vídeo "naturais" (.ce-video-natural): markup sem wrapper absoluto
  // e sem aspect-ratio inline — o vídeo aparece INTEIRO mesmo com CSS antigo em
  // cache no celular (nenhuma regra legada conhece essa classe; só valem os
  // estilos inline + estas regras). Persianas lote 15/09-3.
  rules.push(".ce-video-natural{width:100%!important;max-width:100%!important;height:auto!important;min-height:0!important;aspect-ratio:auto!important;overflow:hidden!important;background:#000!important;box-sizing:border-box!important;}");
  rules.push(".ce-video-natural video,.ce-video-natural iframe{width:100%!important;height:auto!important;min-height:0!important;aspect-ratio:auto!important;object-fit:contain!important;display:block!important;border:0!important;background:#000!important;}");
  rules.push("@media(max-width:768px){.ce-video-natural video{max-height:82vh!important;}}");
  // A5) Liberação de sections/containers que cortam vídeos (195px overflow
  // hidden nas motorizadas — classe marcada no banco, sem depender de JS nem
  // de metadata no iPhone). Especificidade (0,4,1) p/ vencer o CSS da página.
  rules.push("body .ce-video-sec-livre.ce-video-sec-livre.ce-video-sec-livre{height:auto!important;min-height:0!important;max-height:none!important;aspect-ratio:auto!important;overflow:visible!important;display:block!important;}");
  // A4) Persianas fabricantes (Amorim/Rio Flex/Gabriel/Coimbra): os 4 cards com
  // a mesma altura e botões "VER MODELOS" alinhados na base (lote 15/09-3).
  // Botão em absolute-bottom com reserva de padding: funciona em qualquer
  // aninhamento do Elementor e em qualquer CSS em cache.
  rules.push("body .fab-grid{align-items:stretch!important;}");
  rules.push("body .fab-card{position:relative!important;padding-bottom:88px!important;}");
  rules.push("body .fab-card .elementor-widget-button{position:absolute!important;left:20px!important;right:20px!important;bottom:20px!important;z-index:2!important;margin:0!important;}");
  // A4b) Persianas fabricantes REDESIGN (lote fornecedores): foto no topo +
  // "N PRODUTOS" + nome + seta — aplicado via JS que monta .fab-new-body.
  // Aqui só o visual; os textos/botão antigos somem quando .fab-new entra.
  rules.push("body .fab-card.fab-new{position:relative!important;padding:0!important;overflow:hidden!important;background:#171208!important;border:1px solid rgba(194,165,122,.25)!important;border-radius:10px!important;}");
  rules.push("body .fab-card.fab-new > .elementor-widget-wrap > *:not(.fab-new-body){display:none!important;}");
  rules.push("body .fab-card .fab-new-body{display:block!important;}");
  rules.push("body .fab-card .fab-new-link{display:block!important;text-decoration:none!important;color:inherit!important;}");
  rules.push("body .fab-card .fab-new-photo{position:relative!important;display:block!important;aspect-ratio:4/3!important;overflow:hidden!important;background:#241c10!important;}");
  rules.push("body .fab-card .fab-new-photo img{display:block!important;width:100%!important;height:100%!important;object-fit:cover!important;}");
  rules.push("body .fab-card .fab-new-arrow{position:absolute!important;top:10px!important;right:10px!important;display:flex!important;align-items:center!important;justify-content:center!important;width:34px!important;height:34px!important;border-radius:50%!important;border:1px solid rgba(194,165,122,.65)!important;background:rgba(20,15,8,.55)!important;color:#e8d5ae!important;font-size:17px!important;line-height:1!important;}");
  rules.push("body .fab-card .fab-new-meta{display:block!important;padding:16px 16px 18px!important;}");
  rules.push("body .fab-card .fab-new-count{display:block!important;margin-bottom:6px!important;color:#c2a57a!important;font:700 10px/1.4 Arial,sans-serif!important;letter-spacing:2.5px!important;}");
  rules.push("body .fab-card .fab-new-name{display:block!important;color:#f5eed8!important;font-family:'Playfair Display',Georgia,serif!important;font-size:21px!important;line-height:1.25!important;}");
  rules.push("body .fab-card .elementor-widget-button a{width:100%!important;justify-content:center!important;}");
  // F2) Logo Fairmont: placa BEGE grande (como o botão FALAR AGORA) com a logo
  // preta por cima — no chip escuro ela sumia ("tudo preto"). Lote 15/09-5.
  // Escopado à classe marcada no banco (caso-logo-claro) para não afetar as
  // outras logos (Perinatal/Pio XI continuam no chip escuro).
  rules.push("body .caso-logo-img.caso-logo-claro{background:linear-gradient(160deg,#eedcab 0%,#d3a95f 100%)!important;border:1px solid rgba(120,85,20,.55)!important;border-radius:10px!important;min-width:250px!important;min-height:140px!important;padding:20px 30px!important;box-shadow:0 6px 18px rgba(0,0,0,.35)!important;}");
  rules.push("body .caso-logo .caso-logo-img.caso-logo-claro{background:linear-gradient(160deg,#eedcab 0%,#d3a95f 100%)!important;border:1px solid rgba(120,85,20,.55)!important;border-radius:10px!important;min-width:250px!important;min-height:140px!important;padding:20px 30px!important;box-shadow:0 6px 18px rgba(0,0,0,.35)!important;}");
  rules.push("body .caso-logo-img.caso-logo-claro img{height:84px!important;max-height:84px!important;width:auto!important;max-width:260px!important;filter:none!important;opacity:1!important;}");
  rules.push("body .caso-logo .caso-logo-img.caso-logo-claro img{height:84px!important;max-height:84px!important;width:auto!important;max-width:260px!important;filter:none!important;opacity:1!important;}");
  rules.push("@media(max-width:768px){body .caso-logo-img.caso-logo-claro{min-width:200px!important;min-height:104px!important;padding:14px 20px!important;}body .caso-logo-img.caso-logo-claro img{height:40px!important;max-height:40px!important;max-width:180px!important;}}");
  rules.push("@media(max-width:768px){body .caso-logo .caso-logo-img.caso-logo-claro{min-width:200px!important;min-height:104px!important;padding:14px 20px!important;}body .caso-logo .caso-logo-img.caso-logo-claro img{height:40px!important;max-height:40px!important;max-width:180px!important;}}");
  // F3) Demais logos (chip escuro): maiores no mobile (pedido 15/09-6).
  rules.push("@media(max-width:768px){body .caso-logo img,body .caso-logo-img img{height:68px!important;max-height:68px!important;max-width:220px!important;}}");
  // F4) Logos das empresas +70% no mínimo (pedido cliente 15/09-7 — vale para
  // TODAS, inclusive bege). Header pode quebrar linha p/ caber.
  rules.push("body .caso-logo img,body .caso-logo-img img{height:100px!important;max-height:100px!important;width:auto!important;max-width:320px!important;}");
  rules.push("body .caso-logo-img.caso-logo-claro img,body .caso-logo .caso-logo-img.caso-logo-claro img{height:100px!important;max-height:100px!important;max-width:300px!important;}");
  rules.push("body .caso-logo-img.caso-logo-claro,body .caso-logo .caso-logo-img.caso-logo-claro{min-width:320px!important;min-height:150px!important;}");
  rules.push("@media(max-width:768px){body .caso-logo img,body .caso-logo-img img{height:115px!important;max-height:115px!important;max-width:330px!important;}body .caso-logo-img,body .caso-logo .caso-logo-img{min-width:280px!important;}body .caso-logo-img.caso-logo-claro,body .caso-logo .caso-logo-img.caso-logo-claro{min-width:280px!important;min-height:120px!important;}body .caso-logo-img.caso-logo-claro img,body .caso-logo .caso-logo-img.caso-logo-claro img{height:68px!important;max-height:68px!important;max-width:300px!important;}body .caso-header{flex-wrap:wrap!important;gap:18px!important;}}");
  // B) Corporativo: vídeo com capa 16/9 preenchendo sem esticar (capa de carpete = 1º frame; poster editável no editor)
  rules.push(".video-sec .video-wrap{aspect-ratio:16/9!important;height:auto!important;min-height:0!important;overflow:hidden!important;background:#000!important;}");
  rules.push(".video-sec .video-wrap video{width:100%!important;height:100%!important;min-height:0!important;aspect-ratio:16/9!important;object-fit:cover!important;display:block!important;background:#000!important;}");
  rules.push(".video-sec .video-wrap video[poster]{object-fit:cover!important;}");
  // C) (substituída pelas regras empilhadas acima — lote 15/09-2: sem scroll
  // lateral nos passos; mantém só o anti-corte do nº4 no desktop)
  rules.push("@media(max-width:768px){.proc-steps,.processo .steps{overflow:visible!important;}.proc-steps .proc-num,.processo .step-dot{flex-shrink:0!important;}}");
  // D) Home Instalações próprias: 1 foto grande de frente + setas no celular (vira carrossel global)
  rules.push("@media(max-width:768px){.porque .carousel-track:not(.ce-global-carousel-track){display:flex!important;flex-flow:row nowrap!important;grid-template-columns:none!important;overflow-x:auto!important;overflow-y:hidden!important;scroll-snap-type:x mandatory!important;scrollbar-width:none!important;-webkit-overflow-scrolling:touch!important;gap:10px!important;transform:none!important;}.porque .carousel-track:not(.ce-global-carousel-track)::-webkit-scrollbar{display:none!important;}.porque .carousel-track:not(.ce-global-carousel-track)>.carousel-foto{flex:0 0 86%!important;min-width:86%!important;width:86%!important;scroll-snap-align:center!important;aspect-ratio:4/5!important;}}");
  rules.push("@media(max-width:768px){.porque .ce-global-carousel-track>*{width:86%!important;flex:0 0 86%!important;scroll-snap-align:center!important;}}");
  // E) Home Avaliações Google: carrossel com setas no celular
  rules.push("@media(max-width:768px){.depoimentos .dep-grid{display:flex!important;flex-flow:row nowrap!important;grid-template-columns:none!important;overflow-x:auto!important;overflow-y:hidden!important;scroll-snap-type:x mandatory!important;scrollbar-width:none!important;-webkit-overflow-scrolling:touch!important;gap:12px!important;}.depoimentos .dep-grid::-webkit-scrollbar{display:none!important;}.depoimentos .dep-grid>.dep-card{flex:0 0 88%!important;min-width:88%!important;width:88%!important;scroll-snap-align:center!important;}}");
  rules.push(".depoimentos .dep-grid.ce-global-carousel-track{display:flex!important;flex-flow:row nowrap!important;overflow-x:auto!important;overflow-y:hidden!important;scroll-snap-type:x mandatory!important;gap:12px!important;}.depoimentos .dep-grid.ce-global-carousel-track>.dep-card{scroll-snap-align:start!important;}");
  // F) Logos empresas (corporativo): chip ESCURO para as logos brancas/coloridas
  // aparecerem (Pio XI branca sumia no chip creme; Perinatal pequena apagada).
  // Lote 15/09-2. Vence o inline background:#fff salvo no banco.
  rules.push("body .caso-logo{width:auto!important;height:auto!important;background:#1d150c!important;border:1px solid rgba(194,165,122,.45)!important;border-radius:10px!important;padding:18px 26px!important;display:inline-flex!important;align-items:center!important;justify-content:center!important;box-shadow:0 4px 14px rgba(0,0,0,.35)!important;}");
  rules.push("body .caso-logo-img{background:#1d150c!important;border:1px solid rgba(194,165,122,.4)!important;border-radius:10px!important;padding:18px 26px!important;width:auto!important;min-width:200px!important;min-height:110px!important;height:auto!important;box-shadow:0 4px 14px rgba(0,0,0,.35)!important;display:flex!important;align-items:center!important;justify-content:center!important;}");
  rules.push("body .caso-logo .caso-logo-img{background:#1d150c!important;border:1px solid rgba(194,165,122,.4)!important;border-radius:10px!important;padding:18px 26px!important;min-width:200px!important;min-height:110px!important;box-shadow:0 4px 14px rgba(0,0,0,.35)!important;display:flex!important;align-items:center!important;justify-content:center!important;}");
  rules.push(".caso-logo img,.caso-logo-img img{filter:none!important;opacity:1!important;height:64px!important;width:auto!important;max-width:220px!important;max-height:64px!important;object-fit:contain!important;}");
  rules.push("@media(max-width:768px){body .caso-logo-img,body .caso-logo .caso-logo-img{min-width:170px!important;min-height:96px!important;padding:14px 20px!important;}.caso-logo img,.caso-logo-img img{height:58px!important;max-height:58px!important;max-width:190px!important;}}");
  rules.push(".caso-logo-placeholder{background:#1d150c!important;color:#f5eed8!important;border-radius:4px!important;padding:10px 16px!important;}");
  // G) Cortinas/Persianas/Papel: tira borda escura nos cantos dos cards (mantém legível via text-shadow)
  rules.push(".foto-card,.inst-slide,.carousel-foto,.gal-item,.insp-slide,.media-foto{border-radius:0!important;box-shadow:none!important;outline:none!important;}");
  rules.push(".foto-card::before,.foto-card::after,.inst-slide::before,.inst-slide::after,.carousel-foto::before,.carousel-foto::after,.gal-item::before,.gal-item::after{box-shadow:none!important;}");
  rules.push(".foto-card{border:none!important;}");
  // H) Sublinhados: segmentos corporativo + ícone WhatsApp (pisos/arquitetos) sem underline
  rules.push(".intro-pills a,.intro-pill,.intro-segs a,.segmentos a,[class*=segmento] a{text-decoration:none!important;border-bottom:none!important;box-shadow:none!important;}");
  rules.push("a .ti,a i[class*=whatsapp],a i[class*=brand],.wa-float,.wa-float *,.navbar-wa,.navbar-wa *,.nav-cta,.nav-cta *,.btn-talk,.btn-talk *{text-decoration:none!important;border-bottom:none!important;box-shadow:none!important;background-image:none!important;}");
  rules.push("a:has(> i.ti-brand-whatsapp){text-decoration:none!important;}");
  // I) Lupa do header no mobile: ao lado do hamburger (não flutuando sobre o hero)
  rules.push("@media(max-width:768px){.navbar-search{position:absolute!important;right:72px!important;top:60px!important;z-index:11!important;}.navbar-search-toggle{width:42px!important;height:42px!important;}.navbar-search-form{position:fixed!important;top:190px!important;left:12px!important;right:12px!important;width:auto!important;max-width:none!important;}}");
...
function hasCarouselMarkup(html) {
  return /data-ce-widget=["']carousel["']|class=["'][^"']*(?:carousel-track|inst-track|insp-track|gal-track|dep-grid)\b/i.test(html);
}
...
function buildMotVideoScript() {
  return `(() => {
    function fitSlot(video) {
      const slot = video.closest ? video.closest('.mot-video-ph,.mot-video,.ce-video-natural,[data-video-slot]') : null;
      if (!slot) return;
      // Vídeo sempre inteiro, sem corte, em qualquer largura (celular, PC, editor):
      // desfaz o wrapper absoluto 16/9 salvo no banco via estilo inline com
      // !important (vale mesmo onde o CSS global não pegou). Image 3 cliente.
      try {
        const wrap = video.closest ? video.closest('[data-ce-bg-video]') : null;
        if (wrap && slot.contains(wrap)) {
        ...
        // Solta sections ancestrais que cortam o vídeo (ex.: inner-section de
        // 195px com overflow:hidden nas motorizadas da persianas — lote 15/09-6).
        // Só mexe em quem é MENOR que o slot (corta de verdade); para no body.
        try {
    ...
    function fix(video) {
      ...
      fitSlot(video);
      try { video.addEventListener('loadedmetadata', () => fitSlot(video), { once: true }); } catch (e) {}
      video.setAttribute('playsinline', '');
      video.setAttribute('webkit-playsinline', '');
      video.muted = true;
    ...
    function boot() {
      document.querySelectorAll('.mot-video video, .mot-video-ph video, .ce-video-natural video').forEach(fix);
    }
    if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', boot, { once: true });
    else boot();
  })();`;
...
function buildGlobalCarouselScript() {
  return `(() => {
    const candidateSelector = '[data-ce-widget="carousel"],.carousel-track,.inst-track,.insp-track,.gal-track,.dep-grid';
    ...
    function initialize(track, index) {
      ...
      // Home "porque" (Instalações próprias): vira carrossel global com setas — pedido cliente 09/09
...
// Persianas fabricantes REDESIGN: troca o card texto+botão por foto + "N
// PRODUTOS" + nome + seta (mockup cliente, lote fornecedores). Dados
// (contagens do catálogo em 15/09) embutidos; capas via /catalog-media.
function buildFabRedesignScript() {
  return `(() => {
    const SUPPLIERS = [
    ...
    function boot() {
      document.querySelectorAll('.fab-grid .fab-card:not(.fab-new)').forEach((card) => {
        ...
        body.innerHTML = '<a class="fab-new-link" href="' + esc(href) + '" aria-label="' + esc(sup.name) + '">'
          + '<span class="fab-new-photo"><img src="' + esc(sup.cover) + '" alt="' + esc(sup.name) + '" loading="lazy">'
          + '<span class="fab-new-arrow" aria-hidden="true">↗</span></span>'
          + '<span class="fab-new-meta"><span class="fab-new-count">' + sup.count + ' PRODUTOS</span>'
          + '<span class="fab-new-name">' + esc(sup.name) + '</span></span></a>';
...
// Persianas fabricantes: iguala a altura dos .fab-card e cola os botões na
// base via JS (pixel-perfect em qualquer cascata/cache). Roda no load, resize
// e após fontes/imagens — lote 15/09-5.
function buildFabEqualScript() {
  return `(() => {
    function equalize() {
      document.querySelectorAll('.fab-grid').forEach((grid) => {
        ...
        // Agrupa por fileira (offsetTop): igualar fileiras diferentes entre si
        // esticava os quadros demais (bug 15/09-6). Só iguala dentro da fileira.
        const rows = {};
        cards.forEach((card) => {
        ...
        Object.values(rows).forEach((group) => {
          ...
          const buttons = group.map((card) => card.querySelector('.elementor-widget-button a, .fab-btn a, a'));
          let maxBottom = 0;
          const info = group.map((card, i) => {
            const rect = card.getBoundingClientRect();
            const btn = buttons[i];
            const btnRect = btn ? btn.getBoundingClientRect() : null;
            if (rect.bottom > maxBottom) maxBottom = rect.bottom;
            return { card, btn, rect, btnRect };
          });
          info.forEach(({ card, btn, rect, btnRect }) => {
            const grow = maxBottom - rect.bottom;
            if (grow > 1) card.style.setProperty('min-height', (rect.height + grow) + 'px', 'important');
            if (btn && btnRect) {
              const again = card.getBoundingClientRect();
              const need = (again.bottom - 20) - (btn.getBoundingClientRect().bottom);
              if (need > 1) {
                const cur = parseFloat(window.getComputedStyle(btn).marginTop) || 0;
                btn.style.setProperty('margin-top', (cur + need) + 'px', 'important');
              }
            }
          });
        });
    ...
    function boot() {
      ...
      window.setTimeout(equalize, 800);
      window.setTimeout(equalize, 2500);
      if (document.fonts && document.fonts.ready) document.fonts.ready.then(equalize).catch(() => {});
    }
    if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', boot, { once: true });
    else boot();
  })();`;
...
function buildFaqToggleScript() {
  return `(() => {
    function toggleElementorTitle(title) {
      const toggle = title.closest ? title.closest('.elementor-toggle') : null;
      const scope = toggle || document;
      const titles = Array.from(scope.querySelectorAll('.elementor-tab-title'));
    ...
    function collapseAll() {
      document.querySelectorAll('.elementor-toggle').forEach((toggle) => {
        ...
        // O HTML salvo traz style="display:block" nos conteúdos — remove para o
        // CSS global (fechado por padrão, abre no clique) ser o dono do estado.
        toggle.querySelectorAll('.elementor-tab-content').forEach((content) => {
          if (content.style && content.style.display) content.style.removeProperty('display');
        });
      });
    ...
    function onClick(event) {
      const el = event.target && event.target.closest ? event.target.closest('.elementor-tab-title,.elementor-toggle-title,.faq-q') : null;
      if (!el) return;
      const title = el.closest ? (el.closest('.elementor-tab-title') || el) : el;
      if (title.classList && title.classList.contains('elementor-tab-title')) {
        event.preventDefault();
        toggleElementorTitle(title);
        return;
      }
      if (el.classList && el.classList.contains('elementor-toggle-title')) {
        const parent = el.closest('.elementor-tab-title');
        if (parent) { event.preventDefault(); toggleElementorTitle(parent); return; }
      }
      const item = el.closest ? el.closest('.faq-item') : null;
      if (item) { event.preventDefault(); toggleFaqItem(item); }
    }
