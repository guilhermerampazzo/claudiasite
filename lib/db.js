...
export function buildGlobalCss(settings) {
  ...
  // ── FAQ: um único "+" por pergunta — esconde os textos "+"/"−" embutidos
  // (.ce-faq-icon e spans opened/closed do Elementor, ex. cortinas), pois o "+"
  // já é desenhado via ::after no ícone; sem isso aparecem 2 símbolos juntos.
  rules.push(".elementor-toggle .elementor-toggle-icon .ce-faq-icon,.elementor-toggle .elementor-toggle-icon-closed,.elementor-toggle .elementor-toggle-icon-opened{display:none!important;}");
  ...
  rules.push("@media(max-width:768px){.insp-track .insp-slide{flex:0 0 100%!important;width:100%!important;max-width:100%!important;min-width:0!important;scroll-snap-align:center!important;min-height:320px!important;}}");
  rules.push("@media(min-width:769px){.insp-track .insp-slide{min-height:300px!important;}}");
...
function buildMotVideoScript() {
  return `(() => {
    function fitSlot(video) {
      ...
      if (!slot) return;
      // No mobile o CSS deixa o vídeo em tamanho natural (sem corte) — não força aspect-ratio.
    ...
    function boot(){
      document.querySelectorAll('.elementor-toggle').forEach((toggle)=>{
        ...
        titles.forEach((title)=>{
          if(title.dataset.ceFaqTitleReady==='1') return;
          title.dataset.ceFaqTitleReady='1';
