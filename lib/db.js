...
import { dedupeStackedImages, fixAlbumHtml, normalizeIconFont, removeEmptyVideos } from "./html-utils.js";
import { NAVBAR_CSS_LINK, NAVBAR_HTML } from "./navbar.js";
...
// Sem este handler, uma falha de conexão (ex.: banco fora do ar) emite um
// evento "error" no Pool e derruba o processo Node inteiro.
pool.on("error", (error) => {
...
export async function getHomeHeaderHtml() {
  // Cabeçalho único (o da home) — fonte única em lib/navbar.js, igual ao usado
  // por /, /pisos e pelas páginas congeladas do v2. Não sai mais do banco.
  return NAVBAR_HTML;
}
...
export function renderHtml(html, settings = {}, { globalHeaderHtml = null, seo = null } = {}) {
  ...
  // Rede de segurança das páginas de álbum (papéis): colapsa <a> aninhadas
  // (que viram célula vazia no grid) e religa as capas de álbum sem link.
  rendered = fixAlbumHtml(rendered).html;
  ...
  // Cabeçalho único: carrega por último, garantindo o visual da home.
  tags.push(NAVBAR_CSS_LINK);
  // Hero desktop (proporção da capa): precisa vir depois do CSS global.
  tags.push(`<link id="casa-estampa-hero" rel="stylesheet" href="/puck/hero.css">`);
...
function applySiteChrome(html, settings) {
  let rendered = html;
  // Cabeçalho canônico (lib/navbar.js): menu/logo/cores são os da home em
  // todas as páginas — não deixa o chrome legado reescrever nada. O rodapé
  // continua sendo normalizado (fica fora do bloco).
  const headerCanonico = /data-ce-canonical="navbar"/i.test(rendered);
  if (!headerCanonico) {
...
export function buildGlobalCss(settings) {
  ...
  // ── Heroes: capas mobile (≤768px) quando a cliente enviou versão retrato —
  // corporativo, persianas e arquitetos (lote banners 15/09). Desktop usa a
  // foto gravada no banco; home/papéis/cortinas/pisos ainda não têm versão
  // celular (usam a desktop com cover).
  rules.push("@media(max-width:768px){body.ce-slug-corporativo section.hero{background-image:url('/uploads/capas-v2/m-corporativo.jpg')!important;}}");
  rules.push("@media(max-width:768px){body.ce-slug-persianas .elementor-column.hero{background-image:url('/uploads/capas-v2/m-persianas.jpg')!important;}}");
  rules.push("@media(max-width:768px){body.ce-slug-arquitetos-designers section.hero{background-image:url('/uploads/capas-v2/m-arquitetos-designers.jpg')!important;}}");
  ...
  // A4b) (REMOVIDO 15/09-7: redesign foto nos cards da persianas revertido —
  // cliente quis manter o modelo texto+botão; o visual foto+seta foi para as
  // páginas de álbum em lib/catalog-html.js.)
...
// (REMOVIDO 15/09-7: buildFabRedesignScript revertido — ver lib/catalog-html.js.)
