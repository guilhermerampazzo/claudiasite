...
function applySiteChrome(html, settings) {
  let rendered = html;
  rendered = replaceClassElement(rendered, "a", "navbar-logo", (opening, inner) => {
    const linked = setHtmlAttribute(opening, "href", settings.header.logoHref);
    const logo = inner.replace(/<img\b[^>]*>/i, (tag) => setHtmlAttribute(tag, "src", settings.header.logoSrc));
    return `${linked}${logo}</a>`;
  });
