...
export function buildGlobalCss(settings) {
  ...
  rules.push(".hero .hero-actions,.hero .hero-ctas{margin-top:min(26vh,240px)!important;}");
  rules.push(".elementor > section.elementor-top-section:first-of-type .hero-btns{margin-top:min(26vh,240px)!important;}");
  ...
  return `(() => {
    function equalize() {
      document.querySelectorAll('.fab-grid').forEach((grid) => {
        ...
        const buttons = cards.map((card) => card.querySelector('.elementor-widget-button a, .fab-btn a, a'));
        let maxBottom = 0;
        const info = cards.map((card, i) => {
