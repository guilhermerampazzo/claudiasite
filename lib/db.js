...
export function renderHtml(html, settings = {}, { globalHeaderHtml = null, seo = null } = {}) {
  ...
  if (/media-video-box/.test(rendered)) {
...
export function buildGlobalCss(settings) {
  ...
  rules.push(".caso-logo .caso-logo-img{width:150px!important;height:92px!important;flex:0 0 150px!important;}");
  rules.push(".caso-logo:has(.caso-logo-img){width:auto!important;height:auto!important;flex:0 0 auto!important;}");
  rules.push(".caso-logo-img img{filter:none!important;opacity:1!important;}");
  rules.push("@media(max-width:768px){.caso-header{padding-left:20px!important;padding-right:20px!important;gap:16px!important;}}");
  ...
  rules.push("@media(max-width:768px){.media-video-box{aspect-ratio:auto!important;flex:none!important;display:block!important;}.media-video-box video{width:100%!important;height:auto!important;flex:none!important;object-fit:contain!important;display:block!important;}}");
...
function buildMostruarioVideoScript() {
  return `(() => {
    function fitBox(video) {
      const box = video.closest ? video.closest('.media-video-box') : null;
      if (!box) return;
      const apply = () => {
    ...
    function init() { document.querySelectorAll('.media-video-box video').forEach(fitBox); }
