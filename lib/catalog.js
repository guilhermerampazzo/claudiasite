...
export async function syncCatalog({ onProgress = () => {}, products = true } = {}) {
  ...
  try {
    ...
    for (const [index, item] of categories.entries()) {
      ...
      // slug também é único na tabela: remover linha de outro source_id que esteja usando o mesmo slug
      await dbQuery(
        `DELETE FROM catalog_product_categories WHERE category_source_id IN (SELECT source_id FROM catalog_categories WHERE slug=$2 AND source_id<>$1)`,
        [item.id, item.slug]
      );
      await dbQuery(`DELETE FROM catalog_categories WHERE slug=$2 AND source_id<>$1`, [item.id, item.slug]);
      await dbQuery(
        `INSERT INTO catalog_categories
         ...
         ON CONFLICT (source_id) DO UPDATE SET parent_source_id=EXCLUDED.parent_source_id,name=EXCLUDED.name,
           ...
           image_url=COALESCE(NULLIF(EXCLUDED.image_url,''),catalog_categories.image_url),source_url=EXCLUDED.source_url,sort_order=EXCLUDED.sort_order,updated_at=now()`,
    ...
    for (let page = 1; page <= productPages; page += 1) {
      ...
      for (const item of products) {
        // slug também é único na tabela: remover linha de outro source_id que esteja usando o mesmo slug
        await dbQuery(
          `DELETE FROM catalog_product_categories WHERE product_source_id IN (SELECT source_id FROM catalog_products WHERE slug=$2 AND source_id<>$1)`,
          [item.id, item.slug]
        );
        await dbQuery(`DELETE FROM catalog_products WHERE slug=$2 AND source_id<>$1`, [item.id, item.slug]);
        await dbQuery(
...
export async function getCatalogSectionCategories(pageSlug) {
  ...
  if (pageSlug === "home") {
    const result = await dbQuery(
      `SELECT *, (SELECT count(*)::int FROM catalog_categories child WHERE child.parent_source_id=catalog_categories.source_id AND child.active) child_count
       FROM catalog_categories WHERE parent_source_id IS NULL AND active ORDER BY sort_order, name LIMIT 8`
    );
    return result.rows;
  }
  const pathByPage = {
  ...
  if (pageSlug === "pisos") {
    const parents = await Promise.all(["pisos-laminados", "pisos-vinilicos"].map(getCategoryByPath));
    const parentIds = parents.filter(Boolean).map((parent) => parent.source_id);
    if (!parentIds.length) return [];
    const result = await dbQuery(
      `SELECT *,(SELECT count(*)::int FROM catalog_categories child WHERE child.parent_source_id=catalog_categories.source_id AND child.active) child_count
       FROM catalog_categories WHERE parent_source_id = ANY($1::int[]) AND active ORDER BY sort_order LIMIT 16`,
      [parentIds]
    );
    return result.rows;
  }
