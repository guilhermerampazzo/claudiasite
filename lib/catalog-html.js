...
export function renderCategoryPage({ category, children, listing }) {
  const crumbs = category.path.split("/");
  const breadcrumb = crumbs.map((part, index) => `<a href="/categoria-produto/${crumbs.slice(0, index + 1).join("/")}">${escapeHtml(labelFromSlug(part))}</a>`).join("<span>/</span>");
  const cards = children.length ? categoryGrid(children) : productGrid(listing.products);
  return `<section class="ce-category-hero" style="--category-image:url('${catalogImage("categoria", category.source_id, Boolean(category.image_url))}')"><div class="ce-category-shade"></div><div><nav class="ce-breadcrumb"><a href="/catalogo">Catálogo</a><span>/</span>${breadcrumb}</nav><p>CASA ESTAMPA</p><h1>${escapeHtml(category.name)}</h1>${category.description ? `<div class="ce-category-description">${escapeHtml(category.description)}</div>` : ""}</div></section>
