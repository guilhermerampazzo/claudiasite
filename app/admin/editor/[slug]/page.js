      ...
      <div className="editor-layout">
        ...
        <main className="editor-stage">
          <div className="editor-toolbar">
            <div className="editor-page-name"><h1>{page?.title || "Editor"}</h1><span>{isGlobalPart ? "Area compartilhada em todas as paginas" : `/${slug === "amorim" ? "" : slug}`}</span></div>
            <div className="editor-toolbar-actions">
              ...
              <a className="btn btn-icon" href={isGlobalPart ? "/" : `/${slug === "amorim" ? "" : slug}`} target="_blank" title="Ver publicada"><IconExternalLink size={18} /></a>
...
function ElementEditor({ selected, settingsTab, setSettingsTab, updateStyle, updateStyles, updateAttr, setSelected, post, uploadImage, setContainerLayout, addWidget }) {
  ...
  return <div className="element-editor">
    ...
    {settingsTab === "content" ? <div className="settings-body">
      ...
      {isImage ? <><div className="media-preview"><img src={selected.src || "/assets/logo-letra.svg"} alt="Previa da imagem" /></div><label className="field"><span>Endereco da imagem</span><input value={selected.src || ""} onChange={(event) => updateAttr("src", event.target.value)} /></label><label className="field"><span>Texto alternativo</span><input value={selected.alt || ""} placeholder="Descreva a imagem" onChange={(event) => updateAttr("alt", event.target.value)} /></label><label className="field"><span>Ajuste da imagem</span><select value={selected.objectFit || ""} onChange={(event) => updateStyle("objectFit", event.target.value)}><option value="">Automatico</option><option value="cover">Preencher e cortar</option><option value="contain">Mostrar inteira</option><option value="fill">Esticar</option></select></label><label className="upload-field"><IconPhoto size={18} />Enviar nova imagem<input type="file" accept="image/*" onChange={uploadImage} /></label></> : null}
