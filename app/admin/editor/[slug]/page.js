...
function ElementEditor({ selected, settingsTab, setSettingsTab, updateStyle, updateStyles, updateAttr, setSelected, post, uploadImage, uploadCarouselImages, uploadBackgroundImage, uploadBackgroundVideo, aplicarVideoUrl, removerVideoFundo, setContainerLayout, addWidget, uploadVideoSlot, aplicarVideoSlotUrl, removerVideoSlot }) {
  ...
  return <div className="element-editor">
    ...
    {settingsTab === "style" ? <div className="settings-body">
      <div className="control-section"><div className="control-label">Tipografia</div><div className="field-row"><label className="field"><span>Tamanho da letra</span><input value={selected.fontSize || ""} placeholder="Ex.: 16px" onChange={(event) => updateStyle("fontSize", event.target.value)} /></label><label className="field"><span>Peso</span><select value={selected.fontWeight || ""} onChange={(event) => updateStyle("fontWeight", event.target.value)}><option value="">Normal</option><option value="600">Semi-negrito</option><option value="700">Negrito</option><option value="800">Bem negrito</option></select></label><label className="field"><span>Estilo</span><select value={selected.fontStyle || ""} onChange={(event) => updateStyle("fontStyle", event.target.value)}><option value="">Normal</option><option value="italic">Italico</option><option value="normal">Nao italico</option></select></label></div><button className="text-action" onClick={() => updateStyles({ fontWeight: "700" })}>Colocar em negrito</button></div>
...
<style id="ce-editor-style" data-ce-ui="true">
  ...
  [data-ce-editor-placeholder-widget="true"] .elementor-widget-container, [data-ce-editor-placeholder-widget="true"] .elementor-icon-wrapper { display:block !important; position:absolute !important; inset:0 !important; width:100% !important; height:100% !important; margin:0 !important; padding:0 !important; }
  [data-ce-editor-placeholder-widget="true"] img { display:block !important; position:absolute !important; inset:0 !important; width:100% !important; height:100% !important; max-width:none !important; margin:0 !important; padding:0 !important; object-fit:cover !important; }
  ...
  [data-ce-editor-carousel="true"] { overflow-x:auto !important; scroll-behavior:smooth !important; transform:none !important; transition:none !important; scrollbar-width:thin; scrollbar-color:#1689e8 rgba(255,255,255,.24); }
  [data-ce-editor-carousel="true"], [data-ce-editor-carousel="true"] * { animation-play-state:paused !important; }
  .ce-editor-carousel-button { position:absolute;z-index:2147483645;display:grid;place-items:center;width:42px;height:42px;padding:0;color:#fff;background:#1689e8;border:2px solid #fff;border-radius:50%;box-shadow:0 3px 14px rgba(0,0,0,.35);font:700 25px/1 Arial,sans-serif;cursor:pointer; }
  .ce-editor-carousel-button:hover { background:#0874cd;transform:scale(1.05); }
  .ce-editor-carousel-button:focus-visible { outline:3px solid #fff;outline-offset:2px; }
  .ce-editor-carousel-button[hidden] { display:none; }
</style>
...
(() => {
  ...
  const textTags = new Set(["H1","H2","H3","H4","H5","H6","P","SPAN","STRONG","EM","SMALL","A","BUTTON","LABEL","LI","DT","DD","FIGCAPTION","BLOCKQUOTE","INPUT","TEXTAREA","TH","TD"]);
  let selected = null;
  let counter = 1;
  let dropState = null;
  ...
  function ensureEditorPlaceholderImages(){document.querySelectorAll('.ce-exact-icon img,.ce-exact-icon i.ti,.elementor-widget-icon img,.elementor-widget-icon i.ti,.elementor-icon-wrapper i.fas').forEach((image)=>{const widget=image.closest('.ce-exact-icon,.elementor-widget-icon,.elementor-icon-wrapper');if(!widget)return;const host=findPlaceholderHost(widget);if(!host)return;if(window.getComputedStyle(host).backgroundImage.includes('url(')){widget.dataset.ceEditorPlaceholderHidden='true';return;}const hasSiblingPhoto=Array.from(host.querySelectorAll('img,video,iframe')).some((node)=>node!==image&&node.getBoundingClientRect().height>40);if(hasSiblingPhoto){widget.dataset.ceEditorPlaceholderHidden='true';return;}host.dataset.ceEditorPlaceholderHost='true';widget.dataset.ceEditorPlaceholderWidget='true';});}
  ...
  function updateCarouselControls(){document.querySelectorAll('.ce-editor-carousel-button').forEach((button)=>{const track=document.querySelector('[data-ce-id="'+button.dataset.ceCarouselId+'"]');if(!track||!document.contains(track)){button.remove();return;}const rect=track.getBoundingClientRect();const visible=rect.width>0&&rect.height>0&&rect.bottom>0&&rect.top<window.innerHeight;button.hidden=!visible;if(!visible)return;const left=button.dataset.ceCarouselControl==="-1"?rect.left+12:rect.right-54;button.style.left=(Math.max(4,left)+window.scrollX)+'px';button.style.top=(Math.max(4,rect.top+rect.height/2-21)+window.scrollY)+'px';});}
  ...
  function changed(refresh=true){assignIds();ensureEditorPlaceholderImages();ensureEditorCarouselControls();if(refresh)sendTree();positionSelectionUi();window.parent.postMessage({type:"ce-changed"},"*");}
...
<script id="ce-preview-behaviors" data-ce-ui="true">
(() => {
  // Espelha o site público dentro do preview (computador 1280 / celular 390):
  // FAQ abre/fecha com um único "+" e vídeos motorizados aparecem inteiros.
  // data-ce-ui=true => serialize() remove na hora de salvar (não suja o banco).
  function toggleElementorTitle(title) {
    ...
    titles.forEach((other) => { other.classList.remove("elementor-active"); other.setAttribute("aria-expanded", "false"); });
    if (!isOpen) { title.classList.add("elementor-active"); title.setAttribute("aria-expanded", "true"); }
  }
  document.addEventListener("click", (event) => {
    const el = event.target && event.target.closest ? event.target.closest(".elementor-tab-title,.elementor-toggle-title") : null;
    if (!el || el.closest("[data-ce-ui]")) return;
    const title = el.closest(".elementor-tab-title") || el;
