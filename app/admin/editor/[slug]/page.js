...
(() => {
  ...
  function assignIds() { editableElements().forEach((element) => { if (!element.dataset.ceId) element.dataset.ceId = String(counter++); if(!element.dataset.ceWidget&&element.matches(".carousel-track,.inst-track,.insp-track,.gal-track")&&!element.querySelector('[data-ce-widget="carousel"]')){element.dataset.ceWidget="carousel";element.dataset.ceLabel="Carrossel de imagens";} }); }
  function carouselElements(){return Array.from(document.querySelectorAll('[data-ce-widget="carousel"],.carousel-track,.inst-track,.insp-track,.gal-track')).filter((element)=>!isUi(element)&&!(element.matches('.carousel-track,.inst-track,.insp-track,.gal-track')&&!element.dataset.ceWidget&&element.querySelector('[data-ce-widget="carousel"]')));}
  function carouselScrollTarget(element){return [element,...element.querySelectorAll('*')].filter((node)=>!isUi(node)&&node.scrollWidth>node.clientWidth+2).sort((a,b)=>(b.scrollWidth-b.clientWidth)-(a.scrollWidth-a.clientWidth))[0]||element;}
