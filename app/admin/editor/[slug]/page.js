...
const widgetGroups = [
  ...
  },
  {
    ...
    items: [
      ...
      { type: "gallery", label: "Galeria", icon: IconLayoutGrid },
...
const widgetTemplates = {
  ...
  icon: '<span data-ce-widget="icon" role="img" aria-label="Destaque" style="display:inline-block;font-size:42px;line-height:1;">&#10022;</span>',
  image: '<img data-ce-widget="image" src="/assets/logo-letra.svg" alt="" style="display:block;max-width:100%;height:auto;" />',
  carousel: '<div data-ce-widget="carousel" data-ce-label="Carrossel" style="display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:16px;width:100%;"><img src="/assets/logo-letra.svg" alt="" style="display:block;width:100%;aspect-ratio:4/3;object-fit:contain;" /><img src="/assets/logo-letra.svg" alt="" style="display:block;width:100%;aspect-ratio:4/3;object-fit:contain;" /><img src="/assets/logo-letra.svg" alt="" style="display:block;width:100%;aspect-ratio:4/3;object-fit:contain;" /></div>',
  gallery: '<div data-ce-widget="gallery" data-ce-label="Galeria" style="display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px;width:100%;"><img src="/assets/logo-letra.svg" alt="" style="width:100%;aspect-ratio:4/3;object-fit:contain;" /><img src="/assets/logo-letra.svg" alt="" style="width:100%;aspect-ratio:4/3;object-fit:contain;" /><img src="/assets/logo-letra.svg" alt="" style="width:100%;aspect-ratio:4/3;object-fit:contain;" /><img src="/assets/logo-letra.svg" alt="" style="width:100%;aspect-ratio:4/3;object-fit:contain;" /></div>',
  video: '<video data-ce-widget="video" controls style="display:block;width:100%;min-height:240px;border:1px solid currentColor;"><source src="" type="video/mp4" />Seu navegador nao suporta video.</video>',
      ...
      <div className="editor-layout">
        <aside className={`editor-panel ${panel === "edit" ? "panel-edit-mode" : ""}`}>
          ...
          <div className={`editor-panel-scroll ${panel === "edit" ? "is-editing" : ""}`}>
            {panel === "elements" ? (
              <>
                <label className="widget-search"><IconSearch size={17} /><input value={widgetSearch} onChange={(event) => setWidgetSearch(event.target.value)} placeholder="Buscar entre 38 widgets" /></label>
                <p className="panel-help">Arraste para a pagina ou clique para inserir no container selecionado.</p>
...
function ElementEditor({ selected, settingsTab, setSettingsTab, updateStyle, updateStyles, updateAttr, setSelected, post, uploadImage, setContainerLayout, addWidget }) {
  ...
  return <div className="element-editor">
    ...
    {settingsTab === "content" ? <div className="settings-body">
      ...
      {(selected.tag === "img" || selected.tag === "video" || selected.tag === "audio" || selected.src) ? <><label className="field"><span>Endereco da midia</span><input value={selected.src || ""} onChange={(event) => updateAttr("src", event.target.value)} /></label><label className="upload-field"><IconPhoto size={18} />Trocar arquivo<input type="file" accept="image/*,video/*,audio/*" onChange={uploadImage} /></label></> : null}
    ...
    {settingsTab === "style" ? <div className="settings-body">
      <div className="control-section"><div className="control-label">Cores</div><div className="color-row"><label className="field"><span>Texto</span><input type="color" value={selected.color || "#222222"} onChange={(event) => updateStyle("color", event.target.value)} /></label><label className="field"><span>Fundo</span><input type="color" value={selected.backgroundColor || "#ffffff"} onChange={(event) => updateStyle("backgroundColor", event.target.value)} /></label></div><button className="text-action" onClick={() => updateStyle("backgroundColor", "transparent")}>Fundo transparente</button></div>
      <div className="control-section"><div className="control-label">Tipografia</div><div className="field-row"><label className="field"><span>Tamanho</span><input value={selected.fontSize || ""} placeholder="16px" onChange={(event) => updateStyle("fontSize", event.target.value)} /></label><label className="field"><span>Peso</span><select value={selected.fontWeight || ""} onChange={(event) => updateStyle("fontWeight", event.target.value)}><option value="">Herdar</option><option value="300">Leve</option><option value="400">Normal</option><option value="600">Semi-negrito</option><option value="700">Negrito</option></select></label></div><div className="field-row"><label className="field"><span>Altura da linha</span><input value={selected.lineHeight || ""} placeholder="1.6" onChange={(event) => updateStyle("lineHeight", event.target.value)} /></label><label className="field"><span>Espaco entre letras</span><input value={selected.letterSpacing || ""} placeholder="0px" onChange={(event) => updateStyle("letterSpacing", event.target.value)} /></label></div><label className="field"><span>Fonte</span><input value={selected.fontFamily || ""} placeholder="Herdar fonte global" onChange={(event) => updateStyle("fontFamily", event.target.value)} /></label><label className="field"><span>Alinhamento do texto</span><select value={selected.textAlign || ""} onChange={(event) => updateStyle("textAlign", event.target.value)}><option value="">Herdar</option><option value="left">Esquerda</option><option value="center">Centro</option><option value="right">Direita</option><option value="justify">Justificado</option></select></label></div>
      <div className="control-section"><div className="control-label">Borda</div><div className="field-row"><label className="field"><span>Espessura</span><input value={selected.borderWidth || ""} placeholder="1px" onChange={(event) => updateStyles({ borderWidth: event.target.value, borderStyle: event.target.value ? "solid" : "" })} /></label><label className="field"><span>Raio</span><input value={selected.borderRadius || ""} placeholder="0px" onChange={(event) => updateStyle("borderRadius", event.target.value)} /></label></div><label className="field"><span>Cor da borda</span><input type="color" value={selected.borderColor || selected.color || "#222222"} onChange={(event) => updateStyle("borderColor", event.target.value)} /></label></div>
    </div> : null}
    ...
    {settingsTab === "advanced" ? <div className="settings-body"><div className="control-section"><div className="control-label">Espacamento</div><label className="field"><span>Margem externa</span><input placeholder="Ex.: 0 auto 24px" value={selected.margin || ""} onChange={(event) => updateStyle("margin", event.target.value)} /></label><label className="field"><span>Preenchimento interno</span><input placeholder="Ex.: 40px 24px" value={selected.padding || ""} onChange={(event) => updateStyle("padding", event.target.value)} /></label></div><div className="control-section"><div className="control-label">Posicionamento</div><div className="field-row"><label className="field"><span>Posicao</span><select value={selected.position || ""} onChange={(event) => updateStyle("position", event.target.value)}><option value="">Padrao</option><option value="relative">Relativa</option><option value="absolute">Absoluta</option><option value="sticky">Fixa ao rolar</option></select></label><label className="field"><span>Camada (z-index)</span><input value={selected.zIndex || ""} placeholder="1" onChange={(event) => updateStyle("zIndex", event.target.value)} /></label></div><label className="field"><span>Opacidade</span><input type="range" min="0" max="1" step="0.05" value={selected.opacity || "1"} onChange={(event) => updateStyle("opacity", event.target.value)} /><output>{selected.opacity || "1"}</output></label></div><div className="control-section"><div className="control-label">Identificacao</div><label className="field"><span>Classe CSS</span><input value={selected.className || ""} onChange={(event) => updateAttr("class", event.target.value)} /></label></div></div> : null}
...
(() => {
  ...
  window.addEventListener("message",(event)=>{const data=event.data||{};if(data.type==="ce-request-html")window.parent.postMessage({type:"ce-html",html:serialize()},"*");if(data.type==="ce-add-widget")insertWidget(data.widget,{target:currentTarget(),position:canContain(currentTarget())?"inside":"after"});if(data.type==="ce-select-id")select(document.querySelector('[data-ce-id="'+data.id+'"]'));if(!selected)return;if(data.type==="ce-select-parent"&&selected.parentElement!==document.body)select(selected.parentElement);if(data.type==="ce-html-content"){selected.innerHTML=data.value||"";changed();select(selected);}if(data.type==="ce-attr"){if(data.name==="src"&&["VIDEO","AUDIO"].includes(selected.tagName)){let source=selected.querySelector("source");if(!source){source=document.createElement("source");selected.appendChild(source);}source.setAttribute("src",data.value||"");selected.load?.();}else if(data.value)selected.setAttribute(data.name,data.value);else selected.removeAttribute(data.name);changed();select(selected);}if(data.type==="ce-style"){selected.style[data.name]=data.value||"";changed(false);select(selected);}if(data.type==="ce-styles"){Object.entries(data.styles||{}).forEach(([name,value])=>{selected.style[name]=value||"";});changed(false);select(selected);}if(data.type==="ce-delete")deleteSelected();if(data.type==="ce-duplicate")duplicateSelected();});
})();
</script>`;
