...
(() => {
  ...
  function changed(refresh=true){assignIds();ensureEditorCarouselControls();if(refresh)sendTree();positionSelectionUi();window.parent.postMessage({type:"ce-changed"},"*");}
  ...
  function serialize(){const clone=document.documentElement.cloneNode(true);clone.querySelectorAll("[data-ce-id],[data-ce-selected],[data-ce-hover],[data-ce-drop-inside],[data-ce-editor-carousel],[data-ce-editable-text]").forEach((node)=>{node.removeAttribute("data-ce-id");node.removeAttribute("data-ce-selected");node.removeAttribute("data-ce-hover");node.removeAttribute("data-ce-drop-inside");node.removeAttribute("data-ce-editor-carousel");node.removeAttribute("data-ce-editable-text");});clone.querySelectorAll("[data-ce-ui],#ce-editor-style,#ce-editor-bridge,#ce-public-preview-style").forEach((node)=>node.remove());return "<!DOCTYPE html>\\n"+clone.outerHTML;}
  ...
  assignIds();ensureEditorCarouselControls();sendTree();
