      ...
      <main className="admin-main">
        ...
        <section className="site-chrome-section dashboard-section">
          ...
          <form className="chrome-editor" onSubmit={saveSettings}>
            {activeChromeTab === "header" ? <div className="chrome-panel">
              <div className="chrome-preview header-preview"><img src={settings.header.logoSrc} alt="Previa da logo" /><span>{settings.header.ctaLabel}</span></div>
              <div className="chrome-fields"><label className="field"><span>Logo</span><input value={settings.header.logoSrc} onChange={(event) => updateHeader("logoSrc", event.target.value)} /></label><label className="upload-field"><IconPhoto size={18} />Enviar logo<input type="file" accept="image/*,.svg" onChange={uploadLogo} /></label><label className="field"><span>Link da logo</span><input value={settings.header.logoHref} onChange={(event) => updateHeader("logoHref", event.target.value)} /></label><label className="field"><span>Texto do botao</span><input value={settings.header.ctaLabel} onChange={(event) => updateHeader("ctaLabel", event.target.value)} /></label><label className="field chrome-field-wide"><span>Link do botao</span><input value={settings.header.ctaHref} onChange={(event) => updateHeader("ctaHref", event.target.value)} /></label></div>
            </div> : null}
        ...
        <section className="dashboard-section style-settings-section">
          <form className="form-grid" onSubmit={saveSettings}>
            ...
            <div className="style-settings-grid"><div className="global-color-fields"><label className="field"><span>Cor principal</span><input type="color" value={settings.primaryColor} onChange={(event) => setSettings({ ...settings, primaryColor: event.target.value })} /></label><label className="field"><span>Cor de destaque</span><input type="color" value={settings.accentColor} onChange={(event) => setSettings({ ...settings, accentColor: event.target.value })} /></label></div><label className="field"><span>Fonte global</span><select value={settings.fontFamily} style={{ fontFamily: settings.fontFamily }} onChange={(event) => setSettings({ ...settings, fontFamily: event.target.value })}>{GOOGLE_FONTS.map((font) => <option key={font} value={font}>{font}</option>)}</select></label><div className="font-preview" style={{ fontFamily: settings.fontFamily }}><strong>Casa Estampa</strong><span>Interiores com personalidade.</span></div><label className="field"><span>CSS extra</span><textarea value={settings.css} onChange={(event) => setSettings({ ...settings, css: event.target.value })} placeholder=".hero-title { letter-spacing: 1px; }" /></label></div>
